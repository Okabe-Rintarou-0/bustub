...
auto Planner::PlanSelect(const SelectStatement& statement)
    ...
    if (!statement.where_->IsInvalid()) {
        ...
        plan = std::make_shared<FilterPlanNode>(
            std::make_shared<Schema>(schema), std::move(expr), std::move(plan));
    }
    ...
    if (has_window_agg) {
        ...
        if (!statement.group_by_.empty()) {
            throw Exception(
                "Group by is not allowed to use with window function.");
        }
        plan = PlanSelectWindow(statement, std::move(plan));
    } else if (!statement.having_->IsInvalid() ||
               !statement.group_by_.empty() || has_agg) {
        // Plan aggregation
    ...
    if (!statement.sort_.empty()) {
        ...
        for (const auto& order_by : statement.sort_) {
            ...
            order_bys.emplace_back(
                std::make_pair(order_by->type_, abstract_expr));
        }
    ...
    if (!statement.limit_count_->IsInvalid() ||
        ...
        if (!statement.limit_count_->IsInvalid()) {
            if (statement.limit_count_->type_ == ExpressionType::CONSTANT) {
                ...
                if (constant_expr.val_.GetTypeId() == TypeId::INTEGER) {
                    limit =
                        std::make_optional(constant_expr.val_.GetAs<int32_t>());
                } else {
        ...
        if (offset != std::nullopt) {
            throw NotImplementedException(
                "OFFSET clause is not supported yet.");
        }
