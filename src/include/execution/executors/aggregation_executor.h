...
class SimpleAggregationHashTable {
   public:
            ...
            switch (agg_types_[i]) {
                case AggregationType::CountStarAggregate: {
                    result->aggregates_[i] =
                        result->aggregates_[i].Add({TypeId::INTEGER, 1});
                    break;
                }
                case AggregationType::CountAggregate: {
                    Value new_value = input.aggregates_[i];
                    if (new_value.IsNull()) {
                        break;
                    }
                ...
                case AggregationType::SumAggregate: {
                    ...
                    if (result->aggregates_[i].IsNull()) {
                        result->aggregates_[i] = new_value;
                    } else {
                        result->aggregates_[i] =
                            result->aggregates_[i].Add(new_value);
                    }
                    break;
                }
                case AggregationType::MinAggregate: {
                    ...
                    if (result->aggregates_[i].IsNull() ||
                        new_value.CompareLessThan(result->aggregates_[i]) ==
                            CmpBool::CmpTrue) {
                        result->aggregates_[i] = new_value;
                    }
                    break;
                ...
                case AggregationType::MaxAggregate: {
                    ...
                    if (result->aggregates_[i].IsNull() ||
                        new_value.CompareGreaterThan(result->aggregates_[i]) ==
                            CmpBool::CmpTrue) {
                        result->aggregates_[i] = new_value;
                    }
                    break;
    ...
    void InsertEmpty(const AggregateKey& agg_key) {
        if (ht_.count(agg_key) == 0) {
            ht_.insert({agg_key, GenerateInitialAggregateValue()});
        }
    }
