...
class BufferPoolManager {
   public:
     ...
     * a dirty page, you should write it back to the disk first. You also need
     * to reset the memory and metadata for the new page.
     *
     * Remember to "Pin" the frame by calling replacer.SetEvictable(frame_id,
     * false) so that the replacer wouldn't evict the frame before the buffer
     * pool manager "Unpin"s it. Also, remember to record the access history of
     * the frame in the replacer for the lru-k algorithm to work.
     *
     ...
     * @param access_type type of access to the page, only needed for
     * leaderboard tests.
     * @return false if the page is not in the page table or its pin count is <=
     * 0 before this call, true otherwise
     */
    auto UnpinPage(page_id_t page_id,
                   bool is_dirty,
     ...
     * buffer pool, do nothing and return true. If the page is pinned and cannot
     ...
     * page's memory and metadata. Finally, you should call DeallocatePage() to
   ...
   private:
    auto FlushPageInner(page_id_t page_id) -> bool;
    ...
    std::mutex latch_;
    /** This buffer is for the leaderboard task. You may want to use it to
     * optimize the write requests. */
