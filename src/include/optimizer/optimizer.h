...
class Optimizer {
   ...
   private:
     ...
     * aggregation, or when we need to rename the columns in the planner. We
     * merge these projections so as to make execution faster.
     */
     ...
     * nested loop join) and a filter plan node. We can merge the filter
     * condition into nested loop join to achieve better efficiency.
     */
    ...
    /**
     * @brief rewrite expression to be used in nested loop joins. e.g., if we
     * have `SELECT * FROM a, b WHERE a.x = b.y`, we will have `#0.x = #0.y` in
     * the filter plan node. We will need to figure out where does `0.x` and
     * `0.y` belong in NLJ (left table or right table?), and rewrite it as `#0.x
     * = #1.y`.
     *
    ...
    auto GetColumnAndPredKeys(const AbstractExpressionRef& expr,
                              const ColumnValueExpression** column_expr_ptr,
                              std::vector<AbstractExpressionRef>& pred_keys)
        -> bool;

    auto GetColumnEqualExpressionPair(const ComparisonExpression* cmp_expr)
        -> std::optional<
     ...
     * Useful when join reordering. BusTub doesn't support statistics for now,
     * so it's the only way for you to get the table size :(
     *
     * @param table_name
